  ...
  >
    <template #default>
      <div
        ...
        class="text-size-6 absolute left-5 top-1/2 z-10 flex -translate-y-2/4 items-center text-black-1"
      >
        <AppLink
...
import type { ITextButtonHintProps } from '@/components/hints/utils/models';
import BaseHint from '@/components/hints/BaseHint.vue';
...
const setHintHeight = () => {
  if (
    textList.value?.at(currentTextId.value) &&
    messageList.value.at(currentTextId.value) &&
    buttonList.value.at(currentTextId.value)
  ) {
    const maxHeight = Math.max(
      textList.value.at(currentTextId.value).getBoundingClientRect().height,
      buttonList.value.at(currentTextId.value).$el.getBoundingClientRect().height,
    );
    const maxHeightWithPadding = maxHeight + 32;
    messageList.value.at(currentTextId.value).style.height = `${maxHeight}px`;
