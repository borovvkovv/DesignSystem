...
import type { IAccordionWithInputProps } from '@/components/selects/models';
import IconArrowDownTriangle from '@/components/icons/IconArrowDownTriangle.vue';
import AccordionItemWithInput from '@/components/selects/AccordionItemWithInput.vue';
...
describe('Компонент AccordionItemWithInput', () => {
  test('При наведении курсора на заголовок выпадающего списка стрелка подсвечивается синим', async () => {
    ...
    expect(wrapper.findComponent(IconAddPerson).classes()).toContainEqual('text-blueGrey-2');
  });
