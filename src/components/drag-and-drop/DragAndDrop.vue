...
import { removePxMeasureFromValue } from '@/utils/string';
...
const onSwipingHandler = (x: number, y: number) => {
  ...
  const leftContent = x - cursorLocationInContent.value.x - (swipeField?.x ?? 0);
  const topContent = y - cursorLocationInContent.value.y - (swipeField?.y ?? 0);
