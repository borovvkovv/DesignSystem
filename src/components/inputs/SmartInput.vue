  ...
  <BaseSelectDropDown>
    <template #input="{ setDropDownVisibility, isDropDownVisible }">
      <div class="flex flex-col gap-y-6">
        ...
        >
          ...
          <div
            v-if="count"
            class="text-size-4 ml-1 text-grey-2"
          >
            {{ count }}
          </div>
        </div>
    ...
    <template #dropDown="{ registerCallSection, unregisterCallSection, setDropDownVisibility }">
      <ul
        ...
        class="text-size-5 max-h-72 overflow-y-scroll bg-white p-2 shadow-24 scrollbar scrollbar-thumb-blueGrey-5 scrollbar-thumb-rounded scrollbar-w-1 hover:scrollbar-thumb-grey-4 active:scrollbar-thumb-blueGrey-3"
...
import { InputType, type ISmartInputProps } from '@/components/inputs/models';
...
const inputChangeHandler = () => {
  {
    value.value = props.options?.filter((option) => option.title.toLowerCase() === query.value.toLowerCase()).at(0);
  }
};
