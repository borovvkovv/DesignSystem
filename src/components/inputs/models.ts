...
import { type Component, defineAsyncComponent, type InputHTMLAttributes } from 'vue';
import type { Size } from '@/enums';
import type { CalendarType } from '@/components/calendars/utils/models';
import type { ISelectItem, SelectKeyType } from '@/components/selects/models';
import type { AriaAttributes } from '@/components/inputs/default/models';
...
export interface IBaseInput extends /* @vue-ignore */ Omit<InputHTMLAttributes, keyof AriaAttributes> {
...
export const InputComponents: { [key: string]: Component } = {
  BasePasswordInput: defineAsyncComponent(() => import('@/components/inputs/BasePasswordInput.vue')),
  BaseSimpleInput: defineAsyncComponent(() => import('@/components/inputs/BaseSimpleInput.vue')),
};

export interface IMultipleSelectProps extends IBaseSearchInput, ISelectError, IMultiSelect {}

interface IAppTextAreaPropsBase {
  placeholder?: string;
  isDisabled?: boolean;
  rows?: number;
  label?: string;
